...
export function VitalMetricRail({ metrics, observedAt, timeZone }: { metrics: VitalMetricItem[]; observedAt: number; timeZone: string }) {
	...
	return <div className="ts-vital-rail__metrics">{groups.map((group) => <section key={group.label} className="ts-vital-bank" data-bank={group.label.toLowerCase()}><h2>{group.label}</h2><div>{group.keys.flatMap((key) => { const metric = metrics.find((item) => item.key === key); const updated = metric?.observedAt ? Date.parse(metric.observedAt) : observedAt; return metric ? [<a key={metric.key} href={metric.href} className="ts-vital-metric" data-tone={metric.tone}><small>{metric.label}</small><strong>{metric.value}</strong><span>{metric.secondary ?? semanticLabel(metric.semantic)}</span><time dateTime={new Date(updated).toISOString()}>Data {formatTimestamp(updated, { timeZone, style: 'time' })}</time></a>] : []; })}</div></section>)}</div>;
}
