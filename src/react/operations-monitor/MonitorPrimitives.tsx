...
export function OperationsStatusBar({ connection, teamName, logoSrc, workdayTitle, activeWorkdays, activeProviders, executionProviders, timeZone, observedAt, onReconnect, density, onDensityChange }: {
	...
	return <div className="ts-operations-status" data-status={connection}>
		...
		{onDensityChange?<button className="ts-operations-density" type="button" onClick={onDensityChange} aria-expanded={density==='expanded'}>{density==='compact'?'Expand vitals':'Compress vitals'}</button>:null}
...
export function VitalMetricRail({ metrics, observedAt, timeZone }: { metrics: VitalMetricItem[]; observedAt: number; timeZone: string }) {
	...
	return <div className="ts-vital-rail__metrics">{groups.map((group) => <section key={group.label} className="ts-vital-bank" data-bank={group.label.toLowerCase()}><h2>{group.label}</h2><div>{group.keys.flatMap((key) => { const metric = metrics.find((item) => item.key === key); const updated = metric?.observedAt ? Date.parse(metric.observedAt) : observedAt; const context=metric?`${metric.label}: ${metric.value}. ${metric.secondary ?? semanticLabel(metric.semantic)}. Updated ${formatTimestamp(updated,{timeZone,style:'time'})}.`:''; return metric ? [<article key={metric.key} className="ts-vital-metric" data-tone={metric.tone} aria-label={context} title={context}><small>{metric.label}</small><strong>{metric.value}</strong><span>{metric.secondary ?? semanticLabel(metric.semantic)}</span><time dateTime={new Date(updated).toISOString()}>Data {formatTimestamp(updated, { timeZone, style: 'time' })}</time><a className="ts-vital-metric__link" href={metric.href}>Inspect {metric.label}</a></article>] : []; })}</div></section>)}</div>;
}
