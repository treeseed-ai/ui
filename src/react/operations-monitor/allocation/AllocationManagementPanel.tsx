...
export function AllocationManagementPanel({ initialSnapshot, endpoint, csrfToken, realtime }: { initialSnapshot: AllocationSnapshot; endpoint: string; csrfToken: string; realtime: { enabled: boolean; intervalMs: number } }) {
	...
	const [projectId, setProjectId] = useState(initialSnapshot.projects[0]?.id ?? '');
	...
	useEffect(() => {
		...
		setProjectId((current) => resource.data.projects.some((item) => item.id === current) ? current : resource.data.projects[0]?.id ?? '');
	...
	const module = (scope: Scope, eyebrow: string, title: string, content: React.ReactNode, key: string) => <section className="ts-allocation-module" aria-label={title} data-scope={scope}><header><span><small>{eyebrow}</small><strong>{title}</strong></span>{scope === 'portfolio' ? <span><small>Available / remaining</small><b>{duration(resource.data.time.availableSeconds)} / {duration(resource.data.time.remainingSeconds)}</b></span> : null}</header><div className="ts-allocation-module__body">{content}</div><footer><span role="status">{messages[scope]}</span>{resource.data.canManage ? <button type="button" disabled={!dirty[key] || !valid[key]} onClick={() => void save(scope)}>Save</button> : <small>Read only</small>}</footer></section>;
	return <>
