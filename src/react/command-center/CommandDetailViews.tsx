...
export function CommandDetailViews({ detail, timeZone, authoringEndpoint = '', onRefresh }: { detail: CommandEntityDetail; timeZone: string; authoringEndpoint?: string; onRefresh?: () => void }) {
	...
	const [view, setView] = useState<'readability' | 'debug' | 'edit'>(()=>detail.kind==='agent' && typeof location!=='undefined' && new URL(location.href).searchParams.get('view')==='edit'?'edit':'readability');
	...
	return <div className="ts-command-view">
		<div className="ts-command-view__tabs" role="tablist" aria-label="Detail view">
			<button type="button" role="tab" id={readTab} aria-controls={readPanel} aria-selected={view === 'readability'} onClick={() => setView('readability')}><span>{detail.kind === 'proposal' ? 'Review' : detail.kind === 'agent' ? 'Brief' : 'Readability'}</span><small>{detail.kind === 'agent' ? 'Role, relationships and recent activity' : 'Human content and relationships'}</small></button>
			<button type="button" role="tab" id={debugTab} aria-controls={debugPanel} aria-selected={view === 'debug'} onClick={() => setView('debug')}><span>{detail.kind === 'proposal' ? 'Inspector' : detail.kind === 'agent' ? 'Trace' : 'Debug'}</span><small>{detail.kind === 'agent' ? 'Assigned and observed forensic record' : 'Integrity and complete record'}</small></button>
			{authoring && detail.kind !== 'proposal' ? <button type="button" role="tab" id={editTab} aria-controls={editPanel} aria-selected={view === 'edit'} onClick={() => setView('edit')}><span>{detail.kind === 'agent' ? 'Designer' : 'Edit'}</span><small>{detail.kind === 'agent' ? 'Definition, activity profiles and test setup' : 'Canonical TreeDX source'}</small></button> : null}
		</div>
		<div id={view === 'readability' ? readPanel : view === 'debug' ? debugPanel : editPanel} role="tabpanel" aria-labelledby={view === 'readability' ? readTab : view === 'debug' ? debugTab : editTab}>{view === 'readability' ? <ReadView detail={detail} timeZone={timeZone} onRefresh={onRefresh} /> : view === 'debug' ? <DebugView detail={detail} timeZone={timeZone} /> : authoring ? detail.kind === 'agent' ? <AgentDefinitionEditor detail={detail} authoring={authoring} saveEndpoint={authoringEndpoint} onCommitted={onRefresh} /> : <StructuredSourceEditor title={detail.title} description="Edit the canonical repository definition. Validation and expected-base concurrency prevent invalid or stale changes." source={String(authoring.source ?? '')} language={authoring.language === 'yaml' ? 'yaml' : 'mdx'} saveEndpoint={authoringEndpoint} expectedBase={typeof authoring.expectedBase === 'string' ? authoring.expectedBase : null} projectId={String(authoring.projectId ?? detail.projectId ?? '')} projectName={String(authoring.projectName ?? detail.projectName ?? 'Selected project')} path={String(authoring.path ?? '')} /> : null}</div>
	</div>;
}
