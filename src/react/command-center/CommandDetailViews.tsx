...
function RelatedTimeline({ items, timeZone }: { items: CommandEntity[]; timeZone: string }) {
	if (!items.length) return null;
	return <section className="ts-command-reading__conversation" aria-label="Notes and questions">
		<header><span>Linked discussion</span><h3>Notes and questions</h3><p>Open any entry without losing this record.</p></header>
		<ol>{items.map((item) => <li key={`${item.kind}:${item.id}`}><CommandOverlayTrigger target={item}><span className="ts-command-reading__conversation-kind">{item.kind}</span><strong>{item.title}</strong>{item.description ? <p>{item.description}</p> : null}{formattedTime(item.occurredAt, timeZone) ? <time dateTime={item.occurredAt ?? undefined}>{formattedTime(item.occurredAt, timeZone)}</time> : null}<i aria-hidden="true">Open →</i></CommandOverlayTrigger></li>)}</ol>
	</section>;
}

function ReadView({ detail, timeZone }: { detail: CommandEntityDetail; timeZone: string }) {
	const primary = detail.primary;
	const conversation = (detail.related ?? []).filter((item) => item.kind === 'note' || item.kind === 'question');
	const related = (detail.related ?? []).filter((item) => item.kind !== 'note' && item.kind !== 'question');
	const posted = formattedTime(primary?.postedAt ?? detail.occurredAt, timeZone);
	return <div className="ts-command-reading">
		...
		{primary?.facts?.length ? <dl className="ts-command-reading__facts">{primary.facts.map((field) => <div key={field.label}><dt>{field.label}</dt><dd><Value value={field.value} /></dd></div>)}</dl> : null}
		<RelatedTimeline items={conversation} timeZone={timeZone} />
		...
		{related.length ? <section className="ts-command-reading__paths"><header><span>Continue investigating</span><h3>Related work and evidence</h3></header><div>{related.map((item) => <CommandOverlayTrigger key={`${item.kind}:${item.id}`} target={item}><span>{item.kind}</span><strong>{item.title}</strong><i aria-hidden="true">→</i></CommandOverlayTrigger>)}</div></section> : null}
	</div>;
}
...
export function CommandDetailViews({ detail, timeZone, authoringEndpoint }: { detail: CommandEntityDetail; timeZone: string; authoringEndpoint: string }) {
	...
	return <div className="ts-command-view">
		<div className="ts-command-view__tabs" role="tablist" aria-label="Detail view">
			<button type="button" role="tab" id={readTab} aria-controls={readPanel} aria-selected={view === 'readability'} onClick={() => setView('readability')}><span>Readability</span><small>Human content and relationships</small></button>
			<button type="button" role="tab" id={debugTab} aria-controls={debugPanel} aria-selected={view === 'debug'} onClick={() => setView('debug')}><span>Debug</span><small>Integrity and complete record</small></button>
			{authoring ? <button type="button" role="tab" id={editTab} aria-controls={editPanel} aria-selected={view === 'edit'} onClick={() => setView('edit')}><span>Edit</span><small>Canonical TreeDX source</small></button> : null}
		</div>
		<div id={view === 'readability' ? readPanel : view === 'debug' ? debugPanel : editPanel} role="tabpanel" aria-labelledby={view === 'readability' ? readTab : view === 'debug' ? debugTab : editTab}>{view === 'readability' ? <ReadView detail={detail} timeZone={timeZone} /> : view === 'debug' ? <DebugView detail={detail} timeZone={timeZone} /> : authoring ? <StructuredSourceEditor title={detail.title} description="Edit the canonical repository definition. Validation and expected-base concurrency prevent invalid or stale changes." source={String(authoring.source ?? '')} language={authoring.language === 'yaml' ? 'yaml' : 'mdx'} saveEndpoint={authoringEndpoint} expectedBase={typeof authoring.expectedBase === 'string' ? authoring.expectedBase : null} projectId={String(authoring.projectId ?? detail.projectId ?? '')} projectName={String(authoring.projectName ?? detail.projectName ?? 'Selected project')} path={String(authoring.path ?? '')} /> : null}</div>
	</div>;
}
