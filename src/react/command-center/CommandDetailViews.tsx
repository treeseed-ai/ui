...
export function CommandDetailViews({ detail, timeZone, authoringEndpoint = '', onRefresh }: { detail: CommandEntityDetail; timeZone: string; authoringEndpoint?: string; onRefresh?: () => void }) {
	...
	return <div className="ts-command-view">
		<div className="ts-command-view__tabs" role="tablist" aria-label="Detail view">
			...
			{authoring && detail.kind !== 'proposal' ? <button type="button" role="tab" id={editTab} aria-controls={editPanel} aria-selected={view === 'edit'} onClick={() => setView('edit')}><span>{detail.kind === 'agent' ? 'Anatomy' : 'Edit'}</span><small>{detail.kind === 'agent' ? 'Designed intent and generated system facts' : 'Canonical TreeDX source'}</small></button> : null}
		</div>
