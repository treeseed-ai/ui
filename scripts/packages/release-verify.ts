...
function assertTarballContents(packResult) {
  ...
  const requiredFiles = [
    ...
    'dist/astro/forms/actions/Button.astro',
    'dist/astro/forms/fields/MarkdownField.astro',
    'dist/astro/shell/layout/ProductShell.astro',
...
async function smokeInstall(tarballPath) {
  ...
  const smokeScript = `
    ...
    await access(resolve(packageRoot, 'dist/astro/forms/actions/Button.astro'));
    ...
    const theme = defineTheme({ defaultScheme: 'fern', defaultMode: 'system' });
...
async function smokeInstallGitDependency() {
  ...
  await access(resolve(packageInstallRoot, 'dist/astro/site/marketing/Hero.astro'));
